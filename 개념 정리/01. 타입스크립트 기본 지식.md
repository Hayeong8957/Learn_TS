...
(async () => {
	try {
		...
		if (err instanceof CustomError) { // err타입이 CustomError가 아닌 다른 에러가 나는 것을 방지
...
type nonReadonly<T> = { // readonly인 속성 모두 떼고 가져와라
...
const newZerocho: Pick<Profile, 'name' | 'age'> = { //name과 age 가져오기 
	name: 'zerocho',
	age: 29,
}
```
